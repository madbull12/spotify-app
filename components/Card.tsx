...
const Card = ({
  ...
  return (
    <div
      className={`flex ${cardClass} rounded-md group cursor-pointer  `}
      onClick={handlePlay}
    >
      <div
        className={`${imageClass}  overflow-hidden ${
          hidden &&
          "group-hover:scale-110 transition-transform duration-200 ease-in-out"
        } `}
      >
        <Image
          alt="image"
          ...
          height={300}
          objectFit="cover"
          className={`brightness-75 rounded-xl w-full `}
        />
        {hidden && (
          <div className="opacity-0 flex items-center gap-2 group-hover:opacity-100 group-hover:flex absolute z-10 bottom-4 left-2  transition-all duration-200 ease-out ">
      ...
      {!hidden && (
        <div className={`flex flex-col relative w-full p-2 `}>
          ...
          <div
            className={`rounded-full bg-green-500  justify-center items-center absolute bottom-2 right-2 hidden group-hover:flex ${
              !isTopResult ? "w-6 h-6" : "w-12 h-12"
            }`}
          >
            {items.uri === playingTrack.uri && play ? (
              <BsFillPauseFill className="text-white text-xl" />
